...
import { aiDetectionService, AIDetectionResult, APIError } from "@/lib/services/ai-detection.service"
...
export function AIDetectionBadge({ 
  ...
  const handleDetection = async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof APIError ? err.message : "Detection failed"
      ...
      if (err instanceof APIError) {
        if (err.isRateLimited()) {
          ...
          toast({
            ...
            description: err.isConfigurationError() 
              ? "AI detection service is not properly configured." 
  ...
  const getBadgeContent = () => {
    ...
    const badge = aiDetectionService.getConfidenceBadge(result)
    return badge.text
  }
            ...
            <TooltipTrigger asChild>
              <Badge
                ...
                className={`cursor-pointer transition-all hover:scale-105 ${
                  result ? aiDetectionService.getConfidenceBadge(result).color : ''
                }`}
