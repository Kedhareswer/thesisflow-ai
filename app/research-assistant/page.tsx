...
// Direct Nova AI implementation for research ideas
async function generateResearchIdeasWithNovaAI(topic: string, context: string, count: number) {
  // Use server-side API route for Nova AI generation
  const response = await fetch('/api/ai/generate', {
    ...
    body: JSON.stringify({
      prompt: `Generate ${count} innovative research ideas for: "${topic}". ${context ? `Context: ${context}` : ''} Return as JSON: {"ideas": [{"title": "string", "description": "string"}]}`,
      maxTokens: 2000,
      temperature: 0.8
    })
  })
  ...
  if (!response.ok) {
    ...
    throw new Error(`Nova AI generation failed: ${errorData.error || response.statusText}`)
  }

  const data = await response.json()
  if (!data.success) {
    throw new Error(data.error || 'Failed to generate research ideas')
  }

  // Parse the response content for ideas
  try {
    ...
    const ideas = lines
      ...
      .map((line: string) => ({
        ...
        description: 'Research idea generated by Nova AI'
      }))
    
    return {
      ideas: ideas.length > 0 ? ideas : [{ title: 'Research Topic Analysis', description: `Analyze ${topic} using current methodologies` }],
      topic,
      context,
      count,
      timestamp: new Date().toISOString()
...
export default function ResearchAssistant() {
  ...
  const generateIdeas = async () => {
    ...
    try {
      // Direct Nova AI call for research idea generation
      const result = await generateResearchIdeasWithNovaAI(topic, context || "", 5)

      setIdeas(
        result.ideas.map((idea: { title: string; description: string }) => ({
          ...idea,
          saved: false,
        })),
      )
