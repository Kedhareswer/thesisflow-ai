...
export async function POST(req: NextRequest) {
  try {
    ...
    if (textSizeInBytes > maxSizeInBytes) {
      ...
      console.log(`Text too large (${Math.round(textSizeInBytes / 1024)}KB), processing ${chunks.length} chunks sequentially...`)
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        if (chunkSizeInBytes > maxSizeInBytes) {
          console.log(`Chunk ${i + 1} is still too large (${Math.round(chunkSizeInBytes / 1024)}KB), splitting further...`)
          ...
          for (let j = 0; j < subChunks.length; j++) {
            try {
              ...
              const adjustedMatches = subChunkMatches.map((match: any) => ({
                ...
                context: {
                  ...match.context,
                  text: subChunks[j].substring(Math.max(0, match.offset - 20), Math.min(subChunks[j].length, match.offset + match.length + 20))
                }
              }))
          ...
          try {
            ...
            const adjustedMatches = chunkMatches.map((match: any) => ({
              ...
              context: {
                ...match.context,
                text: chunk.substring(Math.max(0, match.offset - 20), Math.min(chunk.length, match.offset + match.length + 20))
              }
            }))
 ...
 */
function splitTextIntoChunks(text: string, maxParagraphs: number = 2): string[] {
