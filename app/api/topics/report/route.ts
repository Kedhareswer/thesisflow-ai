...
export const POST = withTokenValidation(
  ...
  async (userId: string, request: NextRequest): Promise<NextResponse> => {
    try {
      ...
      const curation = await withTimeout(tryNova(curatorUser.content, userId, request.signal), curationBudgetMs, 'Curation')
      ...
      const perSource = await withTimeout(tryNova(analyzerUser.content, userId, request.signal), analysisBudgetMs, 'Analysis')
      ...
      const body = await withTimeout(tryNova(synthUser.content, userId, request.signal), synthesisBudgetMs, 'Synthesis')
